...
constexpr cat2 from_sj(add_lvref<const cat1*> s) noexcept {
  ...
  static_assert(std::signed_integral<cat1>);
  auto c = cat2(*s);
  if (c ^ 0x80) return c;
  else if (!(c & 0x60)) {
    if (c == 0x80) return 0xfffe;
    else if (s[1] < 0x40) return 0xfffe;
    else return table1[c - 0x81][s[1] - 0x40];
  } else if (c < 0xe0) {
    if (c == 0xa0) return 0xfffe;
    else return 0xfec0_c2 + c;
  } else if (c < 0xfd) {
    if (s[1] < 0x40) return 0xfffe;
    else return table2[c - 0xe0][s[1] - 0x40];
  } else return 0xfffe;
}
}
