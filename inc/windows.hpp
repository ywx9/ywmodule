...
inline int4 ClientToScreen(HWND hw, POINT* Point) {
  return ::ClientToScreen(hw, Point);
}
...
inline HWND CreateWindow(
  nat4 ExStyle, const cat2* ClassName, const cat2* WindowName,
  nat4 Style, int4 X, int4 Y, int4 Width, int4 Height,
...
inline int8 DefWindowProc(HWND hw, nat4 Message, nat8 WParam, int8 LParam) {
  return ::DefWindowProcW(hw, Message, WParam, LParam);
}


inline int4 DestroyWindow(HWND hw) {
  return ::DestroyWindow(hw);
}
...
inline void DragFinish(HDROP hDrop) {
  ::DragFinish(hDrop);
}


inline nat4 DragQueryFile(HDROP hDrop, nat4 FileIndex, cat2* FileName, nat4 Size) {
  return ::DragQueryFileW(hDrop, FileIndex, FileName, Size);
}
...
inline int4 GetMessage(MSG* Message, HWND hw, nat4 Min, nat4 Max) {
  return ::GetMessageW(Message, hw, Min, Max);
}
...
inline int4 GetWindowRect(HWND hw, RECT* Rect) {
  return ::GetWindowRect(hw, Rect);
}


inline int4 GetWindowText(HWND hw, cat2* Buffer, int4 Length) {
  return ::GetWindowTextW(hw, Buffer, Length);
}


inline int4 GetWindowTextLength(HWND hw) {
  return ::GetWindowTextLengthW(hw);
}
...
inline int4 PeekMessage(
  MSG* Message, HWND hw, nat4 Min, nat4 Max, nat4 Remove) {
  return ::PeekMessageW(Message, hw, Min, Max, Remove);
}


inline int4 MessageBox(
  HWND hw, const cat2* Text, const cat2* Caption, nat4 Type) {
  return ::MessageBoxW(hw, Text, Caption, Type);
}


inline int4 PostMessage(HWND hw, nat4 Message, nat8 WParam, int8 LParam) {
  return ::PostMessageW(hw, Message, WParam, LParam);
}
...
inline int4 ScreenToClient(HWND hw, POINT* Point) {
  return ::ScreenToClient(hw, Point);
}


inline int8 SendMessage(HWND hw, nat4 Message, nat8 WParam, int8 LParam) {
  return ::SendMessageW(hw, Message, WParam, LParam);
}
...
inline HWND SetFocus(HWND hw) {
  return ::SetFocus(hw);
}


inline int4 SetWindowText(HWND hw, const cat2* Text) {
  return ::SetWindowTextW(hw, Text);
}


inline int4 ShowWindow(HWND hw, int4 Command) {
  return ::ShowWindow(hw, Command);
}
...
inline int4 UpdateWindow(HWND hw) {
  return ::UpdateWindow(hw);
}
...
template<> struct tuple_size<yw::win::RECT> : integral_constant<size_t, 4> {};

template<size_t I>
struct tuple_element<I, yw::win::RECT> : std::type_identity<int4> {};

template<> struct tuple_size<yw::win::POINT> : integral_constant<size_t, 2> {};

template<size_t I>
struct tuple_element<I, yw::win::POINT> : std::type_identity<int4> {};
...
// /// default font handle
// inline const win::HFONT DEFAULT_FONT_HANDLE = [] {
